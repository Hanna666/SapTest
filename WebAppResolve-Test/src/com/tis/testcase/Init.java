...
public class Init {
	...
	public static boolean OpenUrl(String Url) {
		...
		try {
			...
			if (Dtype.equalsIgnoreCase("Google Nexus 5")
					|| Dtype.equalsIgnoreCase("Apple iPhone 6")
					|| Dtype.equalsIgnoreCase("Apple iPhone 5")) {
				MobileEmulation(Dtype);
			}

			else {
				if (Btype.equalsIgnoreCase("Chrome")
						|| Btype.equalsIgnoreCase("chromedriver")) {
					System.setProperty("webdriver.chrome.driver",
							System.getProperty("user.dir")
									+ "\\chromedriver\\chromedriver.exe");
					driver = new ChromeDriver();
					result.print("Launched browser.", "Pass");
					 ...
					 * 
					 * fluentWait = new FluentWait<WebDriver>(driver) .withTimeout(10,
					 * TimeUnit.SECONDS) .pollingEvery(500, TimeUnit.MILLISECONDS)
					 * .ignoring(NoSuchElementException.class);
					 */

				} else if (Btype.equalsIgnoreCase("IE")
						|| Btype.equalsIgnoreCase("Internet Explorer")) {
					...
					cap.setCapability(
							...
							true);
					cap.setCapability(InternetExplorerDriver.NATIVE_EVENTS,
							false);
			...
			// commented
			/*
			 * try { String url_new=driver.getCurrentUrl(); int
			 * statusCode=ApplicationLibrary.getHTTPResponseStatusCode(url_new);
			 * Thread.sleep(3000);
			 * 
			 * if(statusCode == 200) {
			 * 
			 * result.print("URL is opened and have a http STATUS ::"+statusCode,
			 * "Pass"); } else {
			 * 
			 * result.print("URL is opened and have a http Error ::"+statusCode,
			 * "Fail"); driver.close(); } } catch(Exception e) {
			 * result.print("URL is not opened and have a http Error:", "Fail");
			 * driver.close(); } try { driver.navigate().to(Url);
			...
			/*
			 * try {
			 * driver.findElement(By.id("HoneywellEUCookie-CookieDismiss"))
			...
			result.print("Couldnt traverse to the specified URL due to driver Exception");
			// driver.close();
		}
	...
	public static boolean Login(String user, String password) {
		...
		//
		// WebElement oWE =
		// driver.findElement(By.xpath(".//textarea[contains(@id,'txt_mn_symptom_text')]"));
		...
		// //
		// //
		// // selenium.type("//input[contains(@id,'startDateTimeInputDate')]",
		// "2011-05-22 05:33:00");
		//
		// oWE.sendKeys("asfsdfasdfasdf");
		...
		try {
			String[] cmd = {
					System.getProperty("user.dir")
							+ "\\TravelToolExActions\\TravelToolExActions.exe",
					"AuthenticationDialog", user, password };

			// String[] cmd = { System.getProperty("user.dir") +
			// "\\TravelToolExActions\\TravelToolExActions.exe","AuthenticationDialog",user,
			// password};
	...
	public static void TestCaseFunction(String s) {
		...
		try {
			...
			clazz = Class.forName("com.tis.testcase.TIS_WebAppResolve_TestCases");
	...
	public static boolean OpenIE(String Url) {
